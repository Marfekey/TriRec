...
class SASREC(tf.keras.Model):
    ...
    def get_user_item_score(self,dataset,user_id_list, item_list,user_map_dict,item_map_dict,batch_size=128):
        ...
        # Optimization: avoid DataFrame fragmentation by constructing columns at once and merging
        scores_data = return_df['score_array'].tolist()
        scores_df = pd.DataFrame(scores_data, columns=item_list, index=return_df.index)
        
        return_df = pd.concat([return_df.drop('score_array', axis=1), scores_df], axis=1)
        return_df = return_df.reset_index().sort_values(by='user_id').reset_index(drop=True)
