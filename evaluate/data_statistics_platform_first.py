...
class Logger:
    ...
    def __init__(self, log_file=None):
        if log_file is None:
            ...
            log_file = str(ANALYZE_ROOT / "data_statistics_log_first.txt")
...
def compute_ctr_probs(user_id: str, candidates: list[str], user_embs: dict, item_embs: dict) -> list[float]:
    ...
    for iid in candidates:
        ...
        cos = max(min(cos, 1.0), -1.0)  # Mild clipping
...
def calculate_metrics(relevance, ranked_items, target_id, eiu_values=None):
    ...
    # Compute the rank of the target item
    target_rank = None
    ...
    if eiu_values is not None and len(eiu_values) == len(ranked_items):
        ...
        # Target item EIU (if present in the candidates)
        if target_id in ranked_items:
...
def evaluate_recommendation_process_file(file_path, exposure_path=None, user_path=None, item_path=None):
    """Evaluate recommendation_process_alphaXX.jsonl: compute metrics for candidates_before and final_candidates respectively."""
    try:
    ...
    for rec in data:
        # --- Unified handling of target_id ---
        target_item = rec.get('target_item', {})
        target_id = str(target_item.get('id', '')).strip() if isinstance(target_item, dict) else str(target_item).strip()

        # --- 1. Get the Before list (force to string and strip) ---
        candidates_before = [str(x).strip() for x in rec.get('candidates_before', rec.get('original_candidates', []))]
        
        # --- 2. Get the After list (sort and strip) ---
        user_agent_scores = rec.get('user_agent_scores', [])
        if user_agent_scores and isinstance(user_agent_scores, list) and len(user_agent_scores) > 0 and isinstance(user_agent_scores[0], dict):
            # Sort by score descending
            sorted_scores = sorted(user_agent_scores, key=lambda x: x.get('score', 0), reverse=True)
            final_candidates = [str(x.get('id')).strip() for x in sorted_scores]
        else:
            final_candidates = [str(x).strip() for x in rec.get('final_candidates', [])]

        # --- 3. Core fallback logic (use a set for efficient and accurate comparison) ---
        existing_set = set(final_candidates)
        for iid in candidates_before:
            if iid not in existing_set:
                final_candidates.append(iid)
                existing_set.add(iid) # Prevent duplicate additions
        ...
        if candidates_before:
            # Modified: use position-weighted exposure statistics
            ...
            # Record total EIU benefit (before)
            mb['EIU_sum'] = EIU_before_sum
            # Single-item (target) benefit (before)
        ...
        if final_candidates:            
            ...
            relevance_a = [1 if str(item).strip() == target_id else 0 for item in final_candidates]
            ...
            # Record total EIU benefit (after)
            ma['EIU_sum'] = EIU_after_sum
            # Single-item (target) benefit (after)
            ...
            # Record the change in total benefit before/after rerank
            if candidates_before:
...
def evaluate_recommendation_process_dir(dir_path, sample_size=None):
    """Scan .jsonl files containing recommendation_process_alpha in the directory and evaluate them."""
    files = []
    ...
    for alpha, fpath, fname in items:
        try:
            ...
            res['alpha'] = alpha
            results[fname] = res
            logger.log(f"Evaluating file: {fname} (alpha={alpha}) | records: {res['after']['processed_records']}/{res['total_records']}")
            if fairness:
...
def print_recommendation_process_report(results):
    """Print the evaluation report for recommendation_process_alpha*.jsonl (before/after comparison and lift)."""
    if not results:
        logger.log("No evaluable recommendation_process_alpha*.jsonl files found.")
        return
...
def load_item_exposure_train(exposure_path=None) -> dict:
    """Load the exposure of each item in training interactions; return {item_id(str): exposure(float)}"""
    if exposure_path is None:
        exposure_path = get_exposure_path("train")
    try:
        with open(exposure_path, "r", encoding="utf-8") as f:
            data = json.load(f)
        # Unify key type as str
...
def build_popularity_groups(exposure_map: dict, num_groups: int = 8) -> list:
    """
    Split items into popularity groups by ascending exposure (approximate quantile grouping).
    Returns: list[set[str]], each group's set of item ids
    """
...
def train_dist_from_exposure(exposure_map: dict, group_boundaries: list) -> np.ndarray:
    """
    Compute training distribution p_train(g) based on exposure:
    p_train(g) = sum of exposures in this group / total exposure
    """
...
def compute_group_distribution(items, group_boundaries):
    ...
    items: list[str] list of item ids in recommendation result
    group_boundaries: list[set[str]] set of item ids for each group
    Returns: array of proportions per group (length = number of groups)
    """
...
def evaluate_fairness_for_file(file_path, exposure_path=None, num_groups=8):
    ...
    for rec in data:
        # --- Unified handling of target_id ---
        target_item = rec.get('target_item', {})
        target_id = str(target_item.get('id') if isinstance(target_item, dict) else target_item).strip()
        ...
        all_items_in_file.update(final_candidates)

    # Load training exposure and add items "not present in training" with exposure 0
    ...
    # Grouping and training distribution based on the expanded exposure
    ...
    # Accumulate per-K group counts and item-level weighted exposure (for Gini)
    ...
    for rec in data:
        candidates_before = [str(x) for x in rec.get('candidates_before', rec.get('original_candidates', []))]
        user_agent_scores = rec.get('user_agent_scores', [])
        if user_agent_scores and isinstance(user_agent_scores, list) and len(user_agent_scores) > 0 and isinstance(user_agent_scores[0], dict):
            sorted_scores = sorted(user_agent_scores, key=lambda x: x.get('score', 0), reverse=True)
            final_candidates = [str(x.get('id')) for x in sorted_scores]
        else:
        ...
        for k in ks:
            if candidates_before:
                ...
                for r, iid in enumerate(topk, start=1):
                    # Weighted exposure statistics using position bias
    ...
    total_catalog_size = len(exposure_map)
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Data statistics and fairness evaluation for stage-one recommendation process")
    parser.add_argument("--target_file", type=str, required=True,
                        help="Path of the recommendation_process.jsonl file to evaluate")
    parser.add_argument("--log_file", type=str, default=None,
                        help="Path of the log file (optional)")
    args = parser.parse_args()

    # Rebuild the logger based on CLI arguments
    logger = Logger(log_file=args.log_file)
    target_file = args.target_file

    logger.log("Recommendation system data statistics analysis tool")
    logger.log(f"Start evaluating file: {target_file}")

    if os.path.exists(target_file):
        res = evaluate_recommendation_process_file(target_file)
        fairness = evaluate_fairness_for_file(target_file)
        res['fairness'] = fairness
        
        # Print report
        results = {os.path.basename(target_file): res}
        ...
        if fairness:
            ...
            fa = fairness.get('after', {})
            for k in ks:
                d_b = fb.get('DGU', {}).get(k, 0)
                d_a = fa.get('DGU', {}).get(k, 0)
                m_b = fb.get('MGU', {}).get(k, 0)
                m_a = fa.get('MGU', {}).get(k, 0)
                g_b = fb.get('Gini', {}).get(k, 0)
                g_a = fa.get('Gini', {}).get(k, 0)
                logger.log(f"Fairness@{k}: DGU={d_b:.4f}->{d_a:.4f}, MGU={m_b:.4f}->{m_a:.4f}, Gini={g_b:.4f}->{g_a:.4f}, delta_DGU={d_a-d_b:.4f}, delta_Gini={g_a-g_b:.4f}")
    else:
        logger.log(f"File does not exist: {target_file}")
